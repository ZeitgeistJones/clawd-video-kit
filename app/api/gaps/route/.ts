...
export async function POST(req: Request) {
  try {
    ...
    const slimVideos = videos.map((v: any) => `- "${v.title}" | ${v.publishedAt} | ${(v.description || '').slice(0, 200)}`).join('\n')

    const prompt = 'Analyze coverage gaps for the Clawd Explains YouTube channel.\n\nREPOS:\n' + slimRepos + '\n\nVIDEOS:\n' + slimVideos + '\n\nFor each repo classify as uncovered, stale (repo pushed 30+ days after video), or covered. Match loosely on name in title or description.\n\nReturn ONLY valid JSON with this structure: {"gaps":[{"repoName":"name","status":"uncovered","matchedVideo":null,"repoLastPushed":"ISO date","priority":"high"}]}'
